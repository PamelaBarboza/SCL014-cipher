...
desplazamiento = 3;
btn_de_cifrar = document.getElementById("cifrar"); 
btn_de_descifrar= document.getElementById("descifrar");
entrada = document.getElementById("input"); 
salida = document.getElementById("output"); 
...
function cifrar(input, output, desplazamiento){
    ...
    for (var i=0,c; c=input [i]; i+++ {
        let caracter_actual = "",
        indice_actual = aBC.indexOf (c),

        if((indice_actual + desplazamiento) <= aBC.length){
            caracter_actual = caracter_actual.charCodeAt(0),
            resultado += caracter_actual +"-";
        }
        else {
            let indice_actual_temporal = indice_actual + desplazamiento;
            sobrante = indice_actual_temporal - aBC.length;
            caracter_actual = aBC[sobrante];
            caracter_actual = caracter_actual.charCodeAt(0); 
            resultado += caracter_actual + "-";
        }
    }
...
btn_de_cifrar.addEventListener('click', function(){
    cifrar(entrada.value, salida, desplazamiento);
}, false);
*/